...
export const ApplicationStatusDetail = ({
  application,
  locale,
}: {
  ...
  const completed = application.timeline.filter((item) => item.state === "done")
    .length;
          ...
          <div className="grid gap-3 lg:grid-cols-[1.1fr_0.9fr]">
            ...
            <div className="space-y-3 rounded-2xl border border-border/70 bg-card/70 p-4">
              <div className="flex items-center gap-2 text-sm font-semibold text-foreground">
                ...
                <span>{t("summaryCard")}</span>
              </div>
              <div className="space-y-1 text-sm text-muted-foreground">
                <p>{t("dialog.company")}: {application.company}</p>
                <p>{t("dialog.recruiter")}: {application.recruiter}</p>
                <p>{t("dialog.location")}: {application.location}</p>
              </div>
              <Separator />
              <div className="space-y-2 text-sm">
                <p className="font-semibold text-foreground">{t("dialog.actions")}</p>
                <ul className="space-y-2 text-muted-foreground">
                  <li className="flex items-start gap-2">
                    <CheckCircle2 className="mt-0.5 h-4 w-4 text-emerald-600" />
                    <span>{t("dialog.actionOne")}</span>
                  </li>
                  <li className="flex items-start gap-2">
                    <CheckCircle2 className="mt-0.5 h-4 w-4 text-emerald-600" />
                    <span>{t("dialog.actionTwo")}</span>
                  </li>
                </ul>
              </div>
            </div>
